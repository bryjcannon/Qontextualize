...
1. Is it a concrete, verifiable claim?
2. Does it make a specific scientific/technical statement?
...
function findOptimalClaimCount(scoredClaims) {
    ...
    const maxClaims = Math.min(10, scores.length); // Cap at 10 claims
    ...
    for (let i = 3; i < maxClaims; i++) {
        if (diffs[i] < 0.5) { // Threshold for significant difference
            elbowPoint = i;
            break;
        }
    }
